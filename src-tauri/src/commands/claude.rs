...
/// Saves a base64 image to a temporary file and returns the path
#[tauri::command]
pub async fn save_temp_image(base64_data: String, file_name: Option<String>) -> Result<String, String> {
    use std::io::Write;
    use base64::{Engine as _, engine::general_purpose};

    // Extract the actual base64 part if it contains header (data:image/xyz;base64,)
    let start = base64_data.find(",").map(|i| i + 1).unwrap_or(0);
    let base64_content = &base64_data[start..];
    ...
    // Determine path (~/.claude/tmp)
    let home = get_claude_dir().map_err(|e| e.to_string())?;
    ...
    let name = file_name.unwrap_or_else(|| {
        format!("image_{}.png", SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_millis())
    });
